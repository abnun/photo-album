/**
 * cvi_reflex_lib.js 1.3 (19-Jun-2009) (c) by Christian Effenberger 
 * All Rights Reserved. Source: reflex.netzgesta.de
...
var cvi_reflex = {
	...
	add: function(image, options) {
		if(image.tagName.toUpperCase() == "IMG") {
			var defopts = { "tilt" : cvi_reflex.defaultTilt, "height" : cvi_reflex.defaultHeight, "distance" : cvi_reflex.defaultDistance, "transparency" : cvi_reflex.defaultTransparency, "border" : cvi_reflex.defaultBorder, "color" : cvi_reflex.defaultColor, "boxmode" : cvi_reflex.defaultBoxmode, "filter" : cvi_reflex.defaultFilter, "callback" : cvi_reflex.defaultCallback }
			if(options) {
		...
		try {
			...
			if(document.all && document.namespaces && !window.opera) {
				if(canvas.tagName.toUpperCase() == "VAR") {
					...
					if(typeof window[callback]==='function') {window[callback](canvas.id,'cvi_reflex');}
				}
			}else {
				if(canvas.tagName.toUpperCase() == "CANVAS" && canvas.getContext("2d")) {
					...
					img.onload = function() {
						...
						context.scale(1,-1);
						if(alternate) {
							context.drawImage(source,2,2,iw,ih,0,-(boxh-iheit-iheit-idist),ww,ih);
						}else {
